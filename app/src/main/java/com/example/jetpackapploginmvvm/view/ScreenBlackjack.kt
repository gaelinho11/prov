...
fun ScreenBlackjack(
    ...
    Column(
        ...
        if (!state.apostaFeta) {
            ...
            Text(text = "PARTIDA DE BLACKJACK", style = MaterialTheme.typography.headlineMedium)
            Spacer(modifier = Modifier.height(20.dp))

            //ara aqui ja es veuen les cartes de la banca
            Text(text = "CARTES DE LA BANCA", fontWeight = FontWeight.Bold)
            state.cartesBanca.forEach { carta ->
                Text(text = "${carta.nom} (val: ${carta.valor})")
            }
            Text(text = "Puntuació Banca: ${state.puntuacioBanca}")

            Spacer(modifier = Modifier.height(20.dp))

            //mostro les cartes que te
            Text(text = "Les teves cartes:")
            ...
            state.cartesJugador.forEach { carta ->
                Text(text = "${carta.nom} (val: ${carta.valor})") // aqui printejo la carta i el valor, perque si la carta no es un numero el valor no es printeja
            }
            ...
            Text(
                text = state.missatge,
                color = if (state.puntuacioJugador > 21) Color.Red else Color.Black
            )
